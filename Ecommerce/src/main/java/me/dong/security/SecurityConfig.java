...
import me.dong.model.service.LoginUserDetailsService;
    ...
    public static class FormLoginSecurityConfig extends WebSecurityConfigurerAdapter{
        ...
        protected void configure(HttpSecurity http) throws Exception {
            ...
            http.antMatcher("/user/**").authorizeRequests()
                    ...
                    .antMatchers("/user/**").hasAnyAuthority(Authorities.USER)
                    .and()
                    .formLogin().loginPage("/user/login").usernameParameter("j_username").passwordParameter("j_password")
                    .loginProcessingUrl("/user/j_security_check").defaultSuccessUrl("/user/product/list").failureUrl("/user/login?error=true")
                    .and()
                    .csrf().disable();
        }
    }
    ...
    public static class ApiSecurityConfig extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity http) throws Exception {
            http.authorizeRequests()
                    .antMatchers("/join", "/login", "/product/**", "/user/**").permitAll()
                    .antMatchers("/cart/**", "/order/**", "/user/cart/**", "/user/order/**").hasAnyAuthority(Authorities.USER)
                    .and()
                    .httpBasic().and()
                    .logout()
